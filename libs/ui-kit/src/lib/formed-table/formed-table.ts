...
export const TABLE_VALUE_ACCESSOR: any = {
  provide: NG_VALUE_ACCESSOR,
  useExisting: forwardRef(() => FormedTable),
  multi: true
};

@Injectable()
export class FormedTableService {
...
@Component({
  ...
  template: `
    ...
    `,
  providers: [FormedTableService, TABLE_VALUE_ACCESSOR]
})
export class FormedTable implements OnInit, AfterViewInit, AfterContentInit, BlockableUI, OnChanges, ControlValueAccessor {
  ...
  constructor(public el: ElementRef, public zone: NgZone, public tableService: FormedTableService, public cd: ChangeDetectorRef) {}
  ...
  ngOnChanges(simpleChange: SimpleChanges) {
    if (simpleChange.value) {
      ...
      this.onModelChange(simpleChange.value.currentValue)
    }
...
export class TableBody {
  ...
  constructor(public dt: FormedTable) {}
}
...
export class ScrollableView implements AfterViewInit,OnDestroy,AfterViewChecked {
  ...
  constructor(public dt: FormedTable, public el: ElementRef, public zone: NgZone) {
...
@Directive({
  selector: '[pFormedSortableColumn]',
  host: {
...
export class FormedSortableColumn implements OnInit, OnDestroy {
...
@Component({
  selector: 'p-formSortIcon',
  template: `
        <i class="ui-sortable-column-icon pi pi-fw" [ngClass]="{'pi-sort-up': sortOrder === 1, 'pi-sort-down': sortOrder === -1, 'pi-sort': sortOrder === 0}"></i>
    `
})
export class FormedSortIcon implements OnInit, OnDestroy {
...
export class FormedSelectableRow implements OnInit, OnDestroy {
  ...
  constructor(public dt: FormedTable, public tableService: FormedTableService) {
    if (this.isEnabled()) {
...
//
//   constructor(public dt: FormedTable, public editableRow: EditableRow) {}
//
//   @HostListener('click', ['$event'])
//   onClick(event: Event) {
...
//
//   constructor(public dt: FormedTable, @Optional() public editableColumn: EditableColumn, @Optional() public editableRow: EditableRow) { }
//
//   ngAfterContentInit() {
//     this.templates.forEach((item) => {
//       switch (item.getType()) {
...
@NgModule({
  ...
  exports: [FormedTable,SharedModule, FormedSortIcon,FormedSortableColumn, FormedSelectableRow],
  declarations: [FormedTable, ScrollableView,TableBody,FormedSortableColumn, FormedSortIcon, FormedSelectableRow]
})
export class FormedTableModule { }
